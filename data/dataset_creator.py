...
def extract_images(video_list, inputDir, outputDir):
    for video in video_list:
        ...
        outTemp = os.path.join(outDir, os.path.splitext(video)[0])
        ...
        for imageNum, image in enumerate(image_list):
            if (imageNum % clip_size == 0):
                if (imageNum + clip_size - 1 >= len(image_list)):
                    break
                folderNum += 1
                folderName = os.path.join(outputDir, folderNum)
                os.mkdir(folderName)
            folderName = os.path.join(outputDir, folderNum)
...
extract_frames(testNames, videos_folder, extractPath)
create_clips(extractPath, testPath)
extract_frames(trainNames, videos_folder, extractPath)
