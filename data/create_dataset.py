...
#run using command in parent folder: "python data\create_dataset.py --videos_folder Adobe --dataset_folder dataset"
...
def extract_frames(videos, inDir, outDir):
    ...
    for video in videos:
        dirtemp = os.path.join(outDir, os.path.splitext(video)[0])
        if not os.path.isdir(dirtemp):
            os.mkdir(dirtemp)
        retn = os.system('{} -i {} -vf scale={}:{} -vsync 0 -qscale:v 2 {}/%04d.jpg'.format("ffmpeg.exe", os.path.join(inDir, video), args.img_width, args.img_height, dirtemp))
        if retn:
...
def main():
    ...
    #shutil.rmtree(args.dataset_folder, ignore_errors=True)
    ...
    """
    lst = []
    lst.append(args.dataset_folder)
